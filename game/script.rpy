...
label did_fire:
    ...
    G "기척을 죽이고 접근했는데 생각보다 대단하구나! 꼬마야"
    USER "교황? 내가 여기에 있는 걸 어떻게 알았지?"
    G "그거는 네가 더 잘 알지 않을까?"
    USER "글쎄? 나는 모르겠는데?"
    G "그래? 병사들이여 저 녀석들을 제압하라!"
    NARR "수많은 병사들이 나랑 미셸 그리고 메키아를 둘러쌓았다."
    ETC13 "교..교황님.. 약속한..금액은.."
    G "이런 이런 제가 깜빡 했군요. 이 분에게 약속한 금화를 주세요"
    USER "고발자인가....?"
    G "빛의 용사가 돌아오자마자 하는 말이 배신자를 놓쳤다고 했는데, 너를 어떻게 찾을까\n고민하던 순간! 너를 목격했다는 이야기를 듣고 이쪽으로 오는 길에\n네가 있다는 곳에서 강력한 마력이 느껴져서 이렇게 찾아왔지!"
    USER "그래서 여기에서 싸우자는 거야?"
    G "흐음... 마을 사람들에게 피해를 주는 건 싫은데.. 얌전히 데려가야겠다...."
    USER "누가 순수하게 잡혀준대?"
    G "그러면 어디 발버둥 쳐봐라 [[언령 : 꿀어라!]"
    USER "뭐라는..."
    "쿠웅!"
    USER "몸이.. 멋대로.."
    G "예전에 멸망시킨 부족에서 얻은 능력이지"
    USER "능력을... 얻었다고?"
    G "자세한거는 알필요 없다. 어서 끌고 가라!"
    MEKIA "그러면 네녀석이 우리 부족을 멸망시킨 거냐?"
    G "우리 부족....? 오니들이 살았던 곳에 생존자는 빠짐없이 죽이라고 했는데.. 놓친 건가.. 아니면.. 다른 곳에 있었다 던지.. 어찌 됐건 그 녀석들 처럼 이 자리에서 죽여 버리면 그만이지"
    MEKIA "이 자식이!!!"
    G "너도 꿇어라.."
    MEKIA "크윽..."
    G "내가 일어나 라고 하기 전 까지는 그대로 있어야 될 것이야"
    USER "어떻게.. 할 수 없는 건가?"
    G "흐음.... 이 아이는 내가 데려가야 겠네"
    NARR "교황은 미셸을 보며 말을 한다."
    USER "데려가서 무슨 짓을 하려고!"
    G "아직 모르는 건가? 이 아이는 특별한 능력을 가지고 있다.\n잘만하면 이용 할 가치가 있다."
    USER "특별한.. 능력?"
    G "아직 모르는 것 같군"
    USER "미셸은 내 생명의 은인이다. 죽여버린다!"
    G "죽인다라... 손가락 하나 못 움직이는 네가 그런 말을 한다고 무서워할 거 같나?"
    USER "크윽.."
    G "그러면 병사들이여 배신자 용사 [povname]랑 그를 도와준 오니를 처형해라"
    USER "메키아! 좋은 방법 없어?"
    MEKIA "언령에서 풀어날 방법이 있긴해"
    USER "어떤거지?"
    MEKIA "시전자를 죽이거나 아니면 시전자 보다 더 강하거나 둘중 하나야"
    USER "교황보다 더 강해지라고? 이 상황에서?"
    G "이 상황을 극복할 방법은 없다. 이만 여기서 죽어라! 배신자여"
    "챕터.8 각성"
    ETC14 "힘이 필요 하나?"
